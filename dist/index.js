...
class PersistentFile extends node_events.EventEmitter {
  constructor({
    ...
    if (typeof this.hashAlgorithm === 'string') {
      this.hash = crypto__default["default"].createHash(this.hashAlgorithm);
    } else {
      this.hash = null;
    }
  }
  ...
  open() {
    this._writeStream = fs__default["default"].createWriteStream(this.filepath);
...
once$1.exports = wrappy$1(once);
once$1.exports.strict = wrappy$1(onceStrict);
...
/* eslint-disable no-underscore-dangle */
const octetStreamType = 'octet-stream'; // the `options` is also available through the `options` / `formidable.options`

function plugin$3(formidable, options) {
...
/* eslint-disable no-underscore-dangle */
const querystringType = 'urlencoded'; // the `options` is also available through the `this.options` / `formidable.options`
...
class MultipartParser extends node_stream.Transform {
  ...
  _flush(done) {
    if (this.state === STATE.HEADER_FIELD_START && this.index === 0 || this.state === STATE.PART_DATA && this.index === this.boundary.length) {
      ...
      done(new FormidableError(`MultipartParser.end(): stream ended unexpectedly: ${this.explain()}`, malformedMultipart, 400));
    }
  }
...
var MultipartParser$1 = Object.assign(MultipartParser, {
  STATES
});

/* eslint-disable no-underscore-dangle */
const multipartType = 'multipart'; // the `options` is also available through the `options` / `formidable.options`
...
/* eslint-disable no-underscore-dangle */
const jsonType = 'json'; // the `options` is also available through the `this.options` / `formidable.options`
...
const DEFAULT_OPTIONS = {
  ...
  encoding: 'utf-8',
  hashAlgorithm: false,
  uploadDir: os__default["default"].tmpdir(),
...
class IncomingForm extends node_events.EventEmitter {
  constructor(options = {}) {
    ...
    const dir = path__default["default"].resolve(this.options.uploadDir || this.options.uploaddir || os__default["default"].tmpdir());
    this.uploaddir = dir;
  ...
  parse(req, cb) {
    this.req = req; // Setup callback first, so we don't miss anything from data events emitted immediately.

    if (cb) {
      const callback = once$1.exports(dezalgo_1(cb));
      this.fields = {};
      const files = {};
  ...
  _handlePart(part) {
    ...
    part.on('end', () => {
      if (!this.options.allowEmptyFiles && fileSize === 0) {
        this._error(new FormidableError(`options.allowEmptyFiles is false, file size should be greather than 0`, noEmptyFiles, 400));
  ...
  _getExtension(str) {
    ...
    let rawExtname = path__default["default"].extname(basename);
  ...
  _joinDirectoryName(name) {
    const newPath = path__default["default"].join(this.uploadDir, name); // prevent directory traversal attacks

    if (!newPath.startsWith(this.uploadDir)) {
      return path__default["default"].join(this.uploadDir, this.options.defaultInvalidName);
    }
      ...
      this._getNewName = part => {
        ...
        if (part.originalFilename) {
          ...
          ({
            ext,
            name
          } = path__default["default"].parse(part.originalFilename));
